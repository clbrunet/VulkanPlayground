...
void Application::create_voxels_shader_storage_buffer() {
	...
	vox_path = get_asset_path("vox/test_rotations.vox");
	vox_path = get_asset_path("vox/tree.vox");
	vox_path = std::filesystem::path{ "D:/Downloads/castle2.vox" };
	vox_path = std::filesystem::path{ "D:/Downloads/castle3.vox" };
	vox_path = std::filesystem::path{ "D:/Downloads/castle.vox" };
	auto const has_import_succeed = import_vox(vox_path, [&](glm::ivec3 const& vox_full_size) {
		m_octree_depth = glm::log2(std::bit_ceil(static_cast<uint32_t>(glm::max(vox_full_size.x, vox_full_size.y, vox_full_size.z))));
