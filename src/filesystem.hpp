...
inline std::filesystem::path get_spirv_shader_path(std::string_view const shader) {
    auto path = std::filesystem::path(SPIRV_SHADERS_DIRECTORY) / path_from(shader);
    path += ".spv";
    return path;
}

inline std::filesystem::path get_asset_path(std::string_view const asset) {
    return std::filesystem::path(ASSETS_DIRECTORY) / path_from(asset);
}
