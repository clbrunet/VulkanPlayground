...
bool import_vox(std::filesystem::path const& path, std::function<void(glm::ivec3 const&)> const vox_full_size, std::function<void(glm::ivec3 const&)> const import_voxel) {
		...
		if (chunk_id == "SIZE") {
			...
			for (auto i = 0; i < frame_count; ++i) {
				...
				if (rotation_it != std::end(frame_attributes)) {
					...
					if (x_index == 0) {
						std::swap(y_index, z_index);
					} else if (y_index == 0) {
						std::swap(x_index, z_index);
					} else {
						std::swap(x_index, y_index);
					}
					node->m_local_transform[x_index][0] = static_cast<float>(x_sign);
					node->m_local_transform[y_index][1] = static_cast<float>(z_sign);
					node->m_local_transform[z_index][2] = static_cast<float>(y_sign);
	...
	auto model_transforms = std::unordered_multimap<int32_t, glm::mat4>{};
	model_transforms.reserve(std::size(model_sizes));
	auto voxel_begin = glm::ivec3{ std::numeric_limits<int32_t>::max() };
	auto voxel_end = glm::ivec3{ std::numeric_limits<int32_t>::min() };
	auto const voxel_begin_end = [&](auto const& self, Node const& node, glm::mat4 const& parent_transform = glm::identity<glm::mat4>()) -> void {
		...
		for (auto const model_id : node.m_model_ids) {
			...
			auto model_transform = global_transform * glm::translate(glm::mat4{ 1 }, glm::vec3{ model_size / -2 });
			auto model_transform_voxel_end = global_transform * glm::translate(glm::mat4{ 1 }, glm::vec3{ model_size / 2 });
			//std::cout << "voxel_begin_end model transform (au niveau du voxel 0 0 0) " << model_id << " :" << model_transform << std::endl;
			//std::cout << "voxel_begin_end model transform (au niveau du voxel voxel_end voxel_end voxel_end) " << model_id << " :" << model_transform_voxel_end << std::endl;
			voxel_begin = glm::min(voxel_begin, glm::ivec3{ model_transform[3] });
			voxel_begin = glm::min(voxel_begin, glm::ivec3{ model_transform_voxel_end[3] });
			voxel_end = glm::max(voxel_end, glm::ivec3{ model_transform_voxel_end[3] });
			voxel_end = glm::max(voxel_end, glm::ivec3{ model_transform[3] });

			model_transforms.emplace(model_id, model_transform);//CBTODO rename voxel_begin_end
			//model_transforms[model_id] = model_transform;//CBTODO rename voxel_begin_end
		}
		for (auto const& child : node.m_children) {
			self(self, child, global_transform);
		}
	};
	voxel_begin_end(voxel_begin_end, root_node);

	auto const full_size = voxel_end - voxel_begin;
