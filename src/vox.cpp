...
bool import_vox(std::filesystem::path const& path, std::function<void(glm::ivec3 const&)> const vox_full_size, std::function<void(glm::ivec3 const&)> const import_voxel) {
	...
	auto for_each_chunks = [&ifstream](auto const function) {
		...
		if (chunk_id == "SIZE") {
			...
			for (auto i = 0; i < frame_count; ++i) {
				...
				if (rotation_it != std::end(frame_attributes)) {
					...
					constexpr auto vox_to_here_matrix = glm::imat3{
						1, 0, 0,
						0, 0, 1,
						0, 1, 0
					};
					auto const rotation = glm::transpose(vox_to_here_matrix) * vox_rotation * vox_to_here_matrix;
	...
	auto const parse_transforms = [&](auto const& self, Node const& node, glm::imat4 const& parent_transform = glm::identity<glm::imat4>()) -> void {
		...
		for (auto const model_id : node.m_model_ids) {
			...
			//auto model_transform = glm::translate(glm::imat4{ 1 }, glm::vec3{ model_size / -2 }) * global_transform;
			...
			//std::cout << "parse_transforms model transform (au niveau du voxel 0 0 0) " << model_id << " :" << model_transform << std::endl;
			//std::cout << "parse_transforms model transform (au niveau du voxel voxel_end voxel_end voxel_end) " << model_id << " :" << model_transform_voxel_end << std::endl;
			...
			model_transforms.emplace(model_id, model_transform);//CBTODO rename parse_transforms
		}
		for (auto const& child : node.m_children) {
			self(self, child, global_transform);
		}
	};
	parse_transforms(parse_transforms, root_node);
